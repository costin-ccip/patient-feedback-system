...
- [ ] T002 Confirm all three caller flows (M0, M2, M3) and the subflow are OFF
      before editing anything.

## Phase 2: Foundational (blocks all caller edits)

- [ ] T003 Create `issueFeedbackToken` via Built-ins → Developer Tools → Custom
      ...
      Save.
- [ ] T004 If the save rejects `throw` or the `createRecord` options form, apply the
      fallback in `research.md` §3/§4, save, and record the deviation verbatim.
- [ ] T005 Re-read the saved source from the live editor and confirm it matches what
      will be documented.
...
- [ ] T006 [US1] Re-read the live "Call a subflow" parameter values and diff them
      against `data-model.md`'s M3 row; correct data-model.md if they differ.
- [ ] T007 [US1] Delete the "Call a subflow" node; place `issueFeedbackToken` on the
      If-else True branch; map params per data-model.md.
- [ ] T008 [US1] Place Zoho Mail "Send email" after it; configure connection, From,
      To, Subject, HTML body per data-model.md; token merge field
      `${issueFeedbackToken_1.token}` (use the actual output variable name).
- [ ] T009 [US1] Verify: connector count, screenshot, param/body read-back, flow OFF.

### M2 - Session 3 Trigger

- [ ] T010 [US1] Same as T006 for M2.
- [ ] T011 [US1] Same as T007 for M2.
- [ ] T012 [US1] Same as T008 for M2.
- [ ] T013 [US1] Same as T009 for M2.

### M0 - Lost Lead Feedback Token

- [ ] T014 [US1] Values already captured (research.md §0); re-confirm unchanged.
- [ ] T015 [US1] Delete "Call a subflow"; place `issueFeedbackToken` on the trigger's
      output; map params (leadId = `${trigger.id}`, patientId empty).
- [ ] T016 [US1] Place and configure "Send email" per data-model.md's M0 row.
- [ ] T017 [US1] Verify as T009.

### Shared

- [ ] T018 [US2] Open `issueFeedbackToken` in the editor and confirm (without saving
...
- [ ] T019 [US1] Retire the subflow: rename to `[RETIRED] Subflow - Issue Feedback
      Token`, leave OFF, do not delete. Update its description to say it's retired
      and point to this feature.
- [ ] T020 [US1] Confirm no remaining active (non-retired) flow contains "Call a
      subflow".
...
- [ ] T021 [US3] Write `implementation-notes.md` for 002 (as-built: function source,
      per-flow node chains and values, deviations, gotchas).
- [ ] T022 [US3] Update 001 `m0-implementation-notes.md` (component table, §11
...
- [ ] T023 [US3] Update `CLAUDE.md` with the cross-milestone convention (no subflows;
...
- [ ] T024 Commit and push via the device-bridge + `gh` method in CLAUDE.md.
